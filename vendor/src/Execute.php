...
class Execute extends vk_api {
    ...
    public function reply($message, $params = []) {
        if ($this->vk->data != []) {
            return $this->sendMessage($this->vk->data->object->peer_id, $message,  ['disable_mentions' => 1] + $params);
        } else {
            throw new VkApiException('Вк не прислал callback, возможно вы пытаетесь запустить скрипт с локалки');
        }
    }
    
    public function reply2($message, $params = []) {
        if ($this->vk->data != []) {
            return  $this->request('messages.send', ['message' => $message, 'disable_mentions' => 1, 'peer_id' => $this->vk->data->object->peer_id] + $params);
        } else {
            throw new VkApiException('Вк не прислал callback, возможно вы пытаетесь запустить скрипт с локалки');
        }
    }
