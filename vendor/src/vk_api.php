...
class vk_api
{
    ...
    public function reply($message, $params = [])
    {
        if ($this->data != []) {
            ...
            return $this->request('messages.send', ['message' => $message, 'disable_mentions' => 1, 'peer_id' => $this->data->object->peer_id] + $params);
        } else {
    ...
    public function request_($method, $params = array())
    {
        $url = 'https://api.vk.com/method/' . $method;
        $params['v'] = $this->version;
        $params['random_id'] = rand(-2147483648, 2147483647);
    ...
    public function eventAnswerSnackbar($event_id, $peer_id, $user_id, $text)
    {
        return $this->request('messages.sendMessageEventAnswer', [
            'event_id' => $event_id,
            'user_id' => $user_id,
            'peer_id' => $peer_id,
    ...
    public function buttonCallback($text, $color, $payload = null)
    {
        return ['callback', $payload, $text, $color];
    }
     ...
     * @throws VkApiException
     */
    public function uploadImage($id, $local_file_path)
    {
     ...
     * @throws VkApiException
     */
    public function sendFiles($url, $local_file_path, $type = 'file')
    {
        $post_fields = [
     ...
     * @throws VkApiException
     */
    public function saveDocuments($file, $title)
    {
